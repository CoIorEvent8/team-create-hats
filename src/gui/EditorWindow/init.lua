...
function Editor:render()
	...
	return Roact.createElement("Frame", {
		...
		Config = Roact.createElement("ScrollingFrame", {
			...
			Enabled = Roact.createElement(CheckboxInput, {
				...
				callback = function()
					...
					PersistentFolder:Reparent(
						self.state.enabled and workspace:FindFirstChildOfClass("Terrain") or false
					)
				end,
			}),
			...
			ShareFrame = Roact.createElement("Frame", {
				LayoutOrder = 3,
				Size = UDim2.new(1, -40, 0, 30),
				BackgroundTransparency = 1,
			}, {
				...
				ExportConfig = Roact.createElement(ButtonInput, {
					LayoutOrder = 0,
					Size = UDim2.new(0.5, -5, 0, 30),

					Text = "Share Config",
					Color = getColors(self.state.theme, "Green"),
					ChangeMouse = not self.state.sharePopUpShown,

					callback = function()
						self:setState(function(state)
							state.sharePopUpShown = true
							return state
						end)
				...
				ImportConfig = Roact.createElement(ButtonInput, {
					LayoutOrder = 1,
					Size = UDim2.new(0.5, -5, 0, 30),
		...
		ExportConfigPopup = Roact.createElement(PopUp, {
			Position = UDim2.new(0.5, 0, 0.5, 0),
			Size = UDim2.new(0.75, 0, 0.4, 0),
			Theme = self.state.theme,
			Text = "Copy the text below and share it with others! They can use your config by using the Import Config button.",
			CopyText = "this is a long config file xd xd xd",
			OkText = "Done",
			OnScreen = self.state.sharePopUpShown,

			callback = function()
				self:setState(function(state)
					state.sharePopUpShown = false
					return state
				end)
